...
class SoilSPHSolver(SPHSolver):
    def __init__(self, particle_system, TDmethod, density, coh, fric):
        ...
        self.YoungMod = 8e7                # Young’s modulus, Pa
        ...
        self.Depq = self.YoungMod / (1 + self.poi) / (1 - 2 * self.poi) * ti.Matrix(
        ...
        self.Gshear = self.YoungMod / 2 / (1 + self.poi)
        self.Kbulk = self.YoungMod / 3 / (1 - 2 * self.poi)
