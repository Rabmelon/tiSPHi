...
class TmpParticleSystem:
    def __init__(self, world, radius) -> None:
        ...
        self.color = ti.Vector.field(2, dtype=float)
        self.particles_node = ti.root.dense(ti.i, self.particle_max_num)    # 使用稠密数据结构开辟每个粒子数据的存储空间，按列存储
        self.particles_node.place(self.x, self.pos2vis, self.color)
...
def gguishow(case, world, s2w_ratio):
    ...
    while window.running:
        ...
        canvas.lines(case.pos2vis, 0.01, indices=case.line_indices, color=(0.5,1,0.5))
        canvas.circles(case.pos2vis, radius=case.radius * s2w_ratio / max(res), color=(0.5, 0.25, 0.5))
